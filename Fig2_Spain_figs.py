...
def norm_by_percent_alnd(pi_res_file, alnd_per_sample_file,thresh):
    ...
    reads_stats['rds_counts'] = 5000000
...
def do_pca(data,meta, meta_vars, figure_out_path, figure_name_string):
    ...
    for variable in meta_vars:
        ...
        g = sns.scatterplot(x = xa, y=ya, data = x2.round(2), hue='subject_disease_status',palette=dict(PC="salmon", CTR="darkcyan").values(), s=12)
        g.get_legend().remove()
...
def distance_from_centroid(data, meta, figure_out_path, figure_name_string):
    ...
    plt.ylim(0.4,1.2)
    g.set(xticklabels=[])
    fig_name = 'Fig2_' + figure_name_string + 'spain_dist_from_centroid.pdf'
    plt.savefig(join(figure_out_path, fig_name), dpi=300)
...
def number_of_plasmids_per_sample(pres_abs, meta, figure_out_path, figure_name_string):
    ...
    g = sns.boxplot(x = 'subject_disease_status', y = 'no_of_diff_elements', ax = ax, data = plas_sum_df,
                    palette=dict(PC="salmon", CTR="darkcyan"), showfliers = False)
    sns.swarmplot(x = 'subject_disease_status',y = 'no_of_diff_elements', data = plas_sum_df, color='black', s=1.8)
    print(figure_out_path,figure_name_string, 'no of elements')
...
def main(mge_info_file, meta_file, pi_res_file,alnd_per_sample_file, pi_thresh,figures_path):
    ...
    merged = pi_res.T.merge(meta, left_index=True, right_index=True, how='left')
    ...
    pi_res = pi_res.T.loc[pi_res.columns.isin(stool_samples)].T
    pres_abs = (pi_res > 0) * 1
    pres_abs = pres_abs[pres_abs.sum(1) > 0]
    run_analysis(pi_res.loc['phage'], meta, meta_vars, figures_path, 'CNIO_Stool_Phages_samples', pres_abs.loc['phage'])
    run_analysis(pi_res.loc['plasmid'], meta, meta_vars, figures_path, 'CNIO_Stool_Plasmid_samples', pres_abs.loc['plasmid'])

if __name__ == "__main__":
    main('Data/random/mMGE_metadata.csv',
         'Data/metadata/CNIO_metadata.tsv',
         'Data/ICRA/CNIO_mmge_ra_100k_res.pkl',
         'Data/ICRA/CNIO_alnd_rds_per_sample.pkl',
         0.0000001,outdir)
